...
func (rpse *ResetPasswordSendEmail) initInOutEvent(in proto.Message) (inEvent *events.SendResetPasswordEmailRequested, outEvent *events.ResetPasswordEmailSent) {
	inEvent = in.(*events.SendResetPasswordEmailRequested)

	outEvent = new(events.ResetPasswordEmailSent)
	outEvent.EventStatus = new(events.Status)
	outEvent.EventStatus.Timestamp = ptypes.TimestampNow()
	outEvent.SendResetPasswordEmailRequested = inEvent
	outEvent.Uid = inEvent.Uid

	return
}

func (rpse *ResetPasswordSendEmail) userExists(inEvent *events.SendResetPasswordEmailRequested) (*user.User, bool) {
	row, err := rpse.DBO.QueryRow("SELECT uuid,email,password,token,role,phone_number,account_type FROM users WHERE email = ?;", inEvent.Email)
	errorkit.ErrorHandled(err)
...
func (rpse *ResetPasswordSendEmail) profileExists(existedUser *user.User) (*user.Profile, bool) {
	row, err := rpse.DBO.QueryRow("SELECT uuid,full_name,photo,reputation FROM profiles WHERE user_uuid = ?;", existedUser.Uuid)
	errorkit.ErrorHandled(err)
...
func (rp *ResetPassword) initInOutEvent(in proto.Message) (inEvent *events.ResetPasswordRequested, outEvent *events.PasswordReseted) {
	inEvent = in.(*events.ResetPasswordRequested)

	outEvent = new(events.PasswordReseted)
	outEvent.EventStatus = new(events.Status)
	outEvent.EventStatus.Timestamp = ptypes.TimestampNow()
	outEvent.ResetPasswordRequested = inEvent
	outEvent.Uid = inEvent.Uid
...
func (rp *ResetPassword) userExists(resetToken string) (*user.User, bool) {
	...
	row, err := rp.DBO.QueryRow("SELECT email,password,token,role,phone_number,account_type FROM users WHERE uuid = ?", userUUID)
	errorkit.ErrorHandled(err)
